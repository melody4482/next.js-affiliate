...
export const getInfuse = async (startDate, endDate) => {
    return fetch(
            `https://fluent.api.hasoffers.com/Apiv3/json?api_key=4b03091bb3b4f6ede0171f521819b12de0d8ce4eee1f7fb2f0c327232fbd85d2&Target=Affiliate_Report&Method=getStats&fields[]=Offer.name&fields[]=Stat.source&fields[]=Stat.payout&fields[]=Stat.clicks&filters[Stat.date][conditional]=GREATER_THAN_OR_EQUAL_TO&filters[Stat.date][values][]=${startDate}&filters[Stat.date][values][]=${endDate}`,
            { method: 'GET' }
        )
        .then(res => res.json())
        .then(data => {
...
export const getPlug = async (startDate, endDate) => {
    ...
    return fetch(
        `https://securetoken.googleapis.com/v1/token?key=AIzaSyCRYBeb5B5J0EJQr7-631BTwu4f6p9EsKc`,
        {
            ...
            body: JSON.stringify({
                grant_type: 'refresh_token',
                refresh_token: 'AOEOulZYVnczctVS6DlGCj1eKDu4wXWCN0I35wr0vsf0xNv8MjZFZclKWCA8OYk8Cp4XDnjEvNKawaRiUMQ653NlcG1wmRWOvr6uGkGCiB75_ZnX-5fmtJzbGweTjfkwEnHiFBylTsGL08sJ_8GbUxV-oBOu4WtXqQ'
            })
        }
    )
    .then(res => res.json())
        ...
        return fetch(
            `https://theplug-prod.herokuapp.com/api/v1/bqReport?start_date=${startDate}&end_date=${endDate}&timezone=America/New_York&columns=date,campaign,campaign_name,campaign_image_url,media_name,dollars&format=json`,
            {
                ...
                headers: {
                    ...
                    'Authorization': 'bearer eyJhbGciOiJIUzI1NiJ9.NTg1NTI.abImRgZCkT1k9zhTmXeLexc_QpTL3wEUFEze_IiXOvo',
